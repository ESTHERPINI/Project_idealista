...
#title image
title_image = Image.open("./Project/images/fachada.jpg")
st.image(title_image)

#load dataframe
data = pd.read_excel("Project/output/project_dataset.xls")
...
'type of property on sale', 'property is exterior', 'property has parking space'))
...
if option == ''
    #load dataframe
    df_population = pd.read_excel("Project/output/poblacion_distrito.xls")
    fig5, ax = plt.plot(x=df_population.district,y=df_population.population, label='Cummulative pos. test first variant',  linestyle='--')
